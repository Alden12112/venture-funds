    ...
    <div className="page-stack">
      ...
      <section className="content-grid content-grid--two">
        ...
        <article className="panel"><div className="panel__head"><div><h2>{t('ledger.tradeAudit')}</h2><p>{t('ledger.tradeAuditHint')}</p></div></div><div className="table-wrap"><table className="table"><thead><tr><th>{t('ledger.time')}</th><th>{t('ledger.instrument')}</th><th>{t('ledger.action')}</th><th>{t('ledger.lots')}</th><th className="text-end">{t('ledger.referencePrice')}</th><th className="text-end">{t('ledger.pnl')}</th></tr></thead><tbody>
          {trades.data.length ? trades.data.map((trade) => { const closed = trade.action === 'close' || trade.action === 'partial-close' || trade.action === 'liquidation'; return <tr key={trade.id}><td>{formatDateTime(trade.createdAt)}</td><td><strong>{trade.symbol}</strong><div className="text-small text-muted">{trade.side === 'long' ? t('market.long') : t('market.short')}</div></td><td><StatusPill tone={trade.action === 'liquidation' ? 'critical' : closed ? 'info' : 'success'}>{trade.action === 'open' ? t('ledger.open') : trade.action === 'partial-close' ? t('ledger.partialClose') : trade.action === 'close' ? t('ledger.close') : trade.action === 'liquidation' ? t('ledger.liquidation') : t('ledger.riskUpdate')}</StatusPill></td><td>{trade.lots.toFixed(2)}</td><td className="text-end">{trade.price.toFixed(4)}</td><td className={`text-end ${trade.pnl == null ? 'text-muted' : trade.pnl >= 0 ? 'trend trend--up' : 'trend trend--down'}`}>{trade.pnl == null ? '—' : `${trade.pnl >= 0 ? '+' : ''}${trade.pnl.toFixed(2)} U`}</td></tr>; }) : <tr><td colSpan={6}><div className="empty-inline"><span>{t('ledger.noTrades')}</span></div></td></tr>}
        </tbody></table></div></article>
      </section>
    </div>
  );
